...
from util import ensure_args_are_lists
...
class _Hierarchical(object):
    ...
    def log_prior(self):
        ...
        # this is the additional term in the likelihood from the mixed model extension
        for tag in self.tags:
...
class HierarchicalEmissions(_Hierarchical):
    def log_likelihoods(self, data, input, mask, tag, x):
        return self.children[tag].log_likelihoods(data, input, mask, tag, x)
    ...
    # override the generic m_step in class _Hierarchical
    def m_step(self, discrete_expectations, continuous_expectations, datas, inputs, masks, tags, optimizer="adam", num_iters=25, **kwargs):
        optimizer = dict(sgd=sgd, adam=adam)[optimizer]

        def _expected_log_joint(discrete_expectations, continuous_expectations):
            elbo = self.log_prior()
            for data, input, mask, tag, x, (expected_states, _, _) \
                in zip(datas, inputs, masks, tags, continuous_expectations, discrete_expectations):

                    lls = self.children[tag].log_likelihoods(data, input, mask, tag, x)
                    elbo += np.sum(expected_states * lls)
        ...
        def _objective(params, itr):
            self.params = params
            obj = _expected_log_joint(discrete_expectations, continuous_expectations)
            return -obj / T
