...
transition_importance_subject= np.zeros((num_subject, num_transitions, num_roi)) + np.nan
for i in range(num_transitions):
    ...
    for s in range(num_subject):
        ...
        for j in range(len(y_subject)):
            ...
            for t in range(T-1): 
                if (zs[t] == k1) & (zs[t+1] == k2): # identify transition point
                    # backtrack a few time points if applies:
                    for t0 in range(3):
                        if (zs[t-t0] == k1) & (t>=t0):
                            x_t.append(xs[t-t0])
                            y_t.append(ys[t-t0])
        ...
        if len(x_t)==0:                   
            ...
            #transition_importance_subject[s,i,:] = h_odds - h_lesion_odds 

            transition_importance_subject[s,i,:] = np.mean(h_lesion_odds) - h_lesion_odds

with open('summary_data/transition_importance_log_odds_subject.pkl','wb') as f:
    pickle.dump(transition_importance_subject, f)